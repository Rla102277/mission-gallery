...
- **AI enrichment**: title, caption, altText, storyTitle, storyBody have gold sparkle AI buttons (type 'meta' in runAi; reads/writes modal inputs only, keyed by storyEditingId; storyTitle regenerates slug). Server sends `context.entity='photo'` → photo-specific prompt hints for these fields (photo title hint only applies when entity=photo, so work/gallery/page titles are unaffected)
